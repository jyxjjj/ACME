...
func getOrRegisterAccount() error {
	ctx := context.Background()
	wantMail := "mailto:" + email
	if _, err := os.Stat(accountJson); os.IsNotExist(err) {
		return registerAccount(ctx, wantMail)
	} else {
		return loadAccount(ctx, wantMail)
	}
}

func loadAccount(ctx context.Context, wantMail string) error {
	Log.Println("[ACME] Found existing account file:", accountJson)
	acc, err := os.ReadFile(accountJson)
	if err != nil {
		return fmt.Errorf("error while reading account file: %v", err)
	}
	key, err := os.ReadFile(accountKey)
	if err != nil {
		return fmt.Errorf("error while reading account key file: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("error while getting account: %v", err)
	}
...
func registerAccount(ctx context.Context, wantMail string) error {
	...
	if err != nil {
		return fmt.Errorf("error while marshaling account key: %v", err)
	}
	pemBlock := &pem.Block{
		Type:  "EC PRIVATE KEY",
		Bytes: keyBytes,
